...
//    }
    ...
    @PutMapping("/updateStatusBookings/{bookingID}")
    public ResponseEntity<String> updateNannyStatusToBookings(@PathVariable Long bookingID) {
        try {
            bookingqueueService.updateStatusByBookings(bookingID);
            return ResponseEntity.ok("Booking status updated successfully");
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error updating booking status");
    ...
    @PutMapping("/updateStatusCancle/{bookingID}")
    public ResponseEntity<String> updateNannyStatusCancle(@PathVariable Long bookingID) {
        try {
            bookingqueueService.updateStatusByCancle(bookingID);
            return ResponseEntity.ok("Booking status updated successfully");
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error updating booking status");
