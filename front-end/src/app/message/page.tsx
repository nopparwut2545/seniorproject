...
const ChatRoom = () => {
  ...
  const [nannies, setNannies] = useState<String[] >([]);
      ...
      if (decodedToken.a.includes('USER')) {
        const fetchData = async () => {
          try {
            ...
            const response1 = await axios.get(`http://localhost:9000/api/bookingqueue/paid-nannies/${userId}`)
            // const nanny_id = response1.data.id;
             
            // const get_nanny = await axios.get(`http://localhost:9000/api/nannies/getby/${nanny_id}`)
            setCustomer(response.data);
            setNannies(response1.data);
            connect(response.data.username);

            response1.data.forEach((nannyUsername: string) => {
                connect(nannyUsername);
              });
 // Connect using the username from the fetched customer data
            console.log("Nanny:", response1.data.username);
          } catch (err) {
...
//   const connect = (username: string, chatPartnerUsername: string) => {
//     let Sock = new SockJS('http://localhost:9000/ws');
//     stompClient = over(Sock);
//     stompClient.connect({}, () => {
//       onConnected(username);
...
//           switch (decodedToken.a[0]) { // Assuming 'a' contains roles and you're interested in the first role
//             case 'USER':
//               userResponse = await axios.get(`http://localhost:9000/api/customers/${userId}`);
//               const bookingResponse = await axios.get(`http://localhost:9000/api/bookingqueue/getbookingstest/${userId}`);
//               chatPartnerId = bookingResponse.data.nanny_id; // Assuming this is how you get the nanny's ID
//               break;
//             case 'NANNY':
//               userResponse = await axios.get(`http://localhost:9000/api/nannies/getby/${userId}`);
//               // For nannies, determine how to get the chat partner ID
//               break;
//             case 'ADMIN':
//               userResponse = await axios.get(`http://localhost:9000/api/admins/${userId}`);
//               // For admins, determine how to get the chat partner ID
//               break;
//             default:
...
//             let chatPartnerUsername = ''; // Initialize chat partner's username
  
//             if (chatPartnerId) {
//               const chatPartnerResponse = await axios.get(`http://localhost:9000/api/nannies/getby/${chatPartnerId}`);
//               chatPartnerUsername = chatPartnerResponse.data.username;
//             }
  
//             setUserData({ ...userData, receivername: chatPartnerUsername });
//             connect(userResponse.data.username, chatPartnerUsername);
//           }
//         } catch (err) {
//           if (axios.isAxiosError(err)) {
//             setError(err.message);
